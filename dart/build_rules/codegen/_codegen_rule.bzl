...
load(":_codegen_action.bzl", "compute_placeholder_outs", "codegen_action")
...
def codegen_rule(
    ...
    Args:
      ...
      generator_args: Optional. Arguments that are always passed to the codegen
        binary. These will be merged with the generator_args passed by callers
        of the created rule. If any arguments impact the file extensions created
        by the binary they must be included here.
      ...
      input_provider: Optional. If an aspect is provided, this must match the
      ...
      supports_outline_codegen: Whether or not this code generator supports
        ...
        `--outline-only` flag will be passed to the codegen_binary along with
        any other arguments.
      outline_summary_deps: If supports_outline_codegen == True, the summaries
        to pass to the outline codegen action. All transitive summaries of these
        deps will also be available.
    ...
    return rule(
        ...
        attrs = {
            ...
            "_default_generator_args": attr.string_list(
...
def _codegen_impl(ctx):
    ...
    if not (ctx.files.srcs or ctx.files.generate_for or placeholder_outs):
